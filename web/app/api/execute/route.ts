...
export async function POST(request: Request) {
  try {
    ...
    if (!sql || !sessionId) {
      return NextResponse.json({ error: 'Missing sql or sessionId' }, { status: 400 });
    }
    ...
    try {
      ...
      return NextResponse.json({ success: true, rows: resultRows, rowCount, markdown: markdownResult });
